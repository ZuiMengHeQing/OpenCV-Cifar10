...
print(f"检测到的GPU设备: {gpus}")

# 2. 配置GPU显存按需分配（关键！防止TensorFlow一次性占用全部显存）
if gpus:
    try:
        for gpu in gpus:
        ...
        # 显存配置必须在初始化GPU之前执行，否则会报错
        print(f"❌ GPU配置失败: {e}")

# 3. 可选：开启设备放置日志（用于验证模型/数据是否真的在GPU上运行）
...
# 数据预处理（CPU上执行，TensorFlow会自动将数据搬运到GPU）
...
# ====================== 【改动2：可选优化GPU训练】 ======================
# 如果显存不足，可以减小batch_size（比如改为16或8）
# 其他参数无需修改，TensorFlow自动在GPU上执行前向/反向传播
history = model.fit(x_train, y_train,
                    batch_size=32,  # 显存不够时调小这个值
                    epochs=2,
